export default class GS1DigitalLinkToolkit {
					...
					if (a % 2 == 0) {
						...
						if (aikeys.includes(k)) {
							if (this.aiRegex[k].test(results[a])) {
								...
								throw new Error(
									'SYNTAX ERROR: invalid syntax for value of (' + k + ') : ' + results[a]
								);
							}
						}
					}
		...
		if (relevantPathComponents.length == 3 && this.regexSafe64.test(relevantPathComponents[2])) {
			if (this.aiRegex[numericPrimaryIdentifier].test(relevantPathComponents[1])) {
				...
				if (extended) {
					...
					let structuredArray = this.buildStructuredArray(gs1AIarray, otherArray);
					rv.structuredOutput = structuredArray;
					rv.elementStringsOutput = this.gs1compressedDigitalLinkToGS1elementStrings(
						gs1DigitalLinkURI,
						true
					);
				}
	...
	handleEncodings(enc, lengthBits, charstr, binstr) {
		...
		switch (enc) {
			...
			case 1:
				binstr +=
					'001' + lengthBits + this.buildBinaryValue(charstr.toUpperCase(), 4, this.hexAlphabet);
				break;

			case 2:
				binstr +=
					'010' + lengthBits + this.buildBinaryValue(charstr.toUpperCase(), 4, this.hexAlphabet);
				break;
		...
		while (totallength - cursor > 8) {
			...
			if (d1 >= 0 && d2 >= 0 && d1 <= 9 && d2 <= 9) {
				...
				if (this.tableP.hasOwnProperty(h1h2)) {
					...
					throw new Error(
						'Fail: Unsupported AI (reserved range) - no entry in tableP; h1h2=' + h1h2
					);
				}
			} else {
	...
	gs1ElementStringsToCompressedGS1DigitalLink(
		...
		if (uncompressedPrimary) {
			return (
				...
				'/' +
				this.bin2base64(this.compressGS1AIarrayToBinary(separated.nonID, useOptimisations, {}))
			);
		} else {
